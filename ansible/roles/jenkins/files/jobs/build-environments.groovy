...
def pluginList = [ // list of plugins known to build on each platform
  ...
  freebsd10: 'aggregation amqp apache apcups ascent bind ceph contextswitch cpu csv curl curl_json curl_xml dbi df disk dns email exec filecount hddtemp interface ipmi load logfile log_logstash match_empty_counter match_hashed match_regex match_timediff match_value mbmon memcachec memcached memory mqtt multimeter mysql network nginx notify_desktop notify_email ntpd nut olsrd openldap openvpn perl perl pf ping postgresql powerdns processes python redis rrdcached rrdtool sigrok snmp statsd swap syslog table tail tail_csv target_notification target_replace target_scale target_set target_v5upgrade tcpconns teamspeak2 ted threshold tokyotyrant unixsock uptime users uuid varnish virt write_graphite write_http write_log write_mongodb write_redis write_sensu write_tsdb zfs_arc zookeeper',
]
