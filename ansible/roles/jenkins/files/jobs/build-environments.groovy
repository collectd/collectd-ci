...
def pluginList = [ // list of plugins known to build on each platform
  ...
  epel7: 'aggregation amqp apache apcups ascent battery bind ceph cgroups conntrack contextswitch cpu cpufreq csv curl curl_json curl_xml dbi df disk dns drbd email entropy ethstat exec fhcount filecount fscache gmond hddtemp interface ipc ipmi iptables ipvs irq java load logfile log_logstash lvm madwifi match_empty_counter match_hashed match_regex match_timediff match_value mbmon md memcached memory modbus mqtt multimeter mysql netlink network nfs nginx notify_desktop notify_email notify_nagios ntpd numa nut onewire olsrd openldap openvpn perl pinba ping postgresql powerdns processes protocols python redis rrdcached rrdtool sensors serial smart snmp statsd swap syslog table tail tail_csv target_notification target_replace target_scale target_set target_v5upgrade tcpconns teamspeak2 ted thermal threshold turbostat unixsock uptime users uuid varnish virt vmem vserver wireless write_graphite write_http write_log write_redis write_riemann write_sensu write_tsdb xmms zfs_arc zookeeper',
  ...
  epel5: 'aggregation amqp apache apcups ascent battery bind cgroups conntrack contextswitch cpu cpufreq csv curl curl_xml dbi df disk drbd email entropy exec fhcount filecount fscache gmond hddtemp interface ipc ipmi irq java load logfile madwifi match_empty_counter match_hashed match_regex match_timediff match_value mbmon md memcachec memcached memory multimeter mysql netlink network nfs nginx notify_desktop notify_email notify_nagios ntpd numa nut olsrd openldap openvpn perl pinba ping postgresql powerdns processes protocols python rrdtool sensors serial snmp statsd swap syslog table tail tail_csv target_notification target_replace target_scale target_set target_v5upgrade tcpconns teamspeak2 ted thermal threshold unixsock uptime users uuid varnish virt vmem vserver wireless write_graphite write_http write_log write_riemann write_sensu write_tsdb zfs_arc zookeeper',
  freebsd10: 'aggregation amqp apache apcups ascent bind ceph contextswitch cpu csv curl curl_json curl_xml dbi df disk dns email exec filecount gmond hddtemp interface ipmi java load logfile log_logstash match_empty_counter match_hashed match_regex match_timediff match_value mbmon memcachec memcached memory modbus mqtt multimeter mysql network nginx notify_desktop notify_email notify_nagios ntpd nut onewire olsrd openldap openvpn perl pf pinba ping postgresql powerdns processes python redis routeros rrdcached rrdtool sigrok snmp statsd swap syslog table tail tail_csv target_notification target_replace target_scale target_set target_v5upgrade tcpconns teamspeak2 ted threshold tokyotyrant unixsock uptime users uuid varnish virt write_graphite write_http write_kafka write_log write_redis write_riemann write_sensu write_tsdb xmms zfs_arc zookeeper',
]
