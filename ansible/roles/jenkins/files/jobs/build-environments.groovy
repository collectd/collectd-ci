...
def pluginList = [ // list of plugins known to build on each platform
  jessie: 'aggregation amqp apache apcups ascent barometer battery bind ceph cgroups conntrack contextswitch cpu cpufreq csv curl curl_json curl_xml dbi df disk dns drbd email entropy ethstat exec fhcount filecount fscache gmond hddtemp interface ipc ipmi iptables ipvs irq java load logfile log_logstash lvm madwifi match_empty_counter match_hashed match_regex match_timediff match_value mbmon md memcachec memcached memory modbus mqtt multimeter mysql netlink network nfs nginx notify_desktop notify_email ntpd numa nut olsrd onewire openldap openvpn perl perl pinba ping postgresql powerdns processes protocols python redis rrdcached rrdtool sensors serial sigrok smart snmp statsd swap syslog table tail tail_csv target_notification target_replace target_scale target_set target_v5upgrade tcpconns teamspeak2 ted thermal threshold tokyotyrant turbostat unixsock uptime users uuid varnish virt vmem vserver wireless write_graphite write_http write_kafka write_log write_redis write_riemann write_sensu write_tsdb zfs_arc zookeeper',
  ...
  jessie: [
    buildJobs: [
      [
        ...
        buildCommand: "./configure ${defaultConfigureOpts.common} ${defaultConfigureOpts.debian} && make --keep-going V=1; make --keep-going check",
      ...
      [
        ...
        buildCommand: "./configure ${defaultConfigureOpts.common} CC=clang && make --keep-going CC='clang -Wall -Werror' V=1; make --keep-going check",
        teardownTask: "PLUGIN_LIST=\"${pluginList.jessie}\"; ${defaultTeardownTask}",
        artifacts: ['collectd-${COLLECTD_BUILD}/**/test_*.log', 'collectd-${COLLECTD_BUILD}/src/config.h', 'collectd-${COLLECTD_BUILD}/config.log', 'dpkg-l.txt'],
      ...
      [
        ...
        buildCommand: "./configure ${defaultConfigureOpts.common} CC=clang && make --keep-going CC='clang -Wall -Werror -Wextra -Wpedantic -Wconversion -Wformat=2 -Wshadow -Wunreachable-code' V=1 || exit 0; make --keep-going check || exit 0",
        warning: ['Clang (LLVM based)'],
      ],
      [
        archs: ['amd64'],
        ...
        buildCommand: "scan-build --keep-going -o ./scan-build ./configure ${defaultConfigureOpts.common} && scan-build --keep-going -o ./scan-build make V=1",
  ...
  epel7: [
    buildJobs: [
      [
        ...
        buildCommand: "./configure ${defaultConfigureOpts.common} ${defaultConfigureOpts.redhat} && make --keep-going V=1; make --keep-going check",
  ...
  freebsd10: [
    buildJobs: [
      [
        ...
        buildCommand: "./configure ${defaultConfigureOpts.common} ${defaultConfigureOpts.freebsd} && make -k V=1; make -k check",
